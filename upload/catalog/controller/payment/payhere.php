...
class ControllerPaymentPayHere extends Controller {
	public function index() {
		...
		$data['testmode'] = $this->config->get('payhere_test');
		if (!$this->config->get('payhere_test')) {
			$data['action'] = 'https://www.payhere.lk/pay/checkout';
		} else {
			$data['action'] = 'https://sandbox.payhere.lk/pay/checkout';
		}

		$data['merchant_id'] = $this->config->get('payhere_merchant_id');
		...
		$data['status_url'] = $this->url->link('payment/payhere/callback');
	...
	public function callback() {
		...
		if ($order_info) {
			...
			if ($this->config->get('payhere_secret')) {
                ...
                $hash .= $this->request->post['status_code'];
                $hash .= strtoupper(md5($this->config->get('payhere_secret')));
                
				$md5hash = strtoupper(md5($hash));
				$md5sig = $this->request->post['md5sig'];

				if (($md5hash != $md5sig) || (strtolower($this->request->post['merchant_id']) != strtolower($this->config->get('payhere_merchant_id'))) || ((float)$this->request->post['payhere_amount'] != $this->currency->format((float)$order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false))) {
					$verified = false;
				}
			} 

			if ($verified) {
				switch($this->request->post['status_code']) {
					case '2':
						$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payhere_order_status_id'), '', true);
						break;
					case '0':
						$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payhere_pending_status_id'), '', true);
						break;
					case '-1':
						$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payhere_canceled_status_id'), '', true);
						break;
					case '-2':
						$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payhere_failed_status_id'), '', true);
						break;
					case '-3':
						$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payhere_chargeback_status_id'), '', true);
						break;
				}
			} else {
